    ...
    try:
        ...
        for i in df2.itertuples():
            ...
            try:
                ...
                pass
            roundDiff.append(j[7] - j[3])
        CTpercent = float(sum(CTpercent)) / len(CTpercent)
        Tpercent = float(sum(Tpercent)) / len(Tpercent)
        roundDiff = float(sum(roundDiff)) / len(roundDiff)
        return [roundDiff, Tpercent, CTpercent, Tpercent * CTpercent]
    except Exception:
        return 0
...
if __name__ == '__main__':
    ...
    dfread['CT%'] = df['c']
    dfread['Comp'] = df['d']
    dfread[['RoD','T%','CT%','Comp']].to_csv("..\\Generated\\All RoundStats Diff.csv", index=False, header=None)
